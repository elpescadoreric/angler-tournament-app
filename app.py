...
if st.session_state.logged_user is None:
    ...
    # High-visibility Submit Catch button top left
    col_submit, col_title = st.columns([1, 5])
    with col_submit:
        if st.session_state.role == "Captain":
            if st.button("🎣 SUBMIT CATCH", type="primary", use_container_width=True):
                st.session_state.active_tab = "Submit Catch"
                st.rerun()
    with col_title:
        st.write("")  # Spacer
    ...
    # Submit Catch form (only for Captains)
    if st.session_state.role == "Captain":
...
st.caption("Everyday Angler App – Your home for charter tournaments | Tight lines!")        'bio': "",
        'county': "",
        'state': "Florida",
        'events': []
    }
